...
import { z } from "zod";
export const APPLICATION_KIND_VALUES = ['new_registration', 'renewal', 'add_rooms', 'delete_rooms', 'cancel_certificate'];
...
const serviceContextSchema = z.object({
    ...
    requestedDeletions: z.array(z.object({
...
export const insertHomestayApplicationSchema = createInsertSchema(homestayApplications, {
    ...
    const totalBeds = singleRooms * (data.singleBedBeds ?? 0) +
...
export const applicationActions = pgTable("application_actions", {
    ...
    issuesFound: jsonb("issues_found").$type(), // List of issues if sending back for corrections
    createdAt: timestamp("created_at").defaultNow(),
});
